...
class GrepTest(unittest.TestCase):
    ...
    # -----------------------------------------------------------------
    # Custom tests (rt) — driving out a bug in _line_match where the
    # `-i` flag short-circuits and ignores `-v` and `-x` combinations.
    # -----------------------------------------------------------------

    # -i + -v: should invert a case-insensitive match.
    # Pattern "FORBIDDEN" (case-insensitive) matches only line 2 of
    # paradise-lost.txt, so -v should return every OTHER line.
    def test_rt_case_insensitive_inverted_match(self, mock_file, mock_open):
        self.assertMultiLineEqual(
            grep("FORBIDDEN", "-i -v", ["paradise-lost.txt"]),
    ...
    # -i + -x: full-line match must still be enforced when -i is on.
    # "FORBIDDEN" is only a *substring* of line 2, never a full line,
    # so the correct result is an empty string. The bug will incorrectly
    # return the substring-matching line.
    def test_rt_case_insensitive_full_line_no_match(self, mock_file, mock_open):
        self.assertMultiLineEqual(
            grep("FORBIDDEN", "-i -x", ["paradise-lost.txt"]),
            "",
        )

    # -i + -x: a genuine full-line match in a different case should
    # still be returned. This verifies -i is honored when combined with -x.
    def test_rt_case_insensitive_full_line_match(self, mock_file, mock_open):
        self.assertMultiLineEqual(
            grep(
                "of mans first disobedience, and the fruit",
                "-i -x",
    ...
    # -i + -x + -v: invert a case-insensitive full-line match.
    # The pattern matches line 1 (case-insensitively), so -v should
    # return every other line.
    def test_rt_case_insensitive_full_line_inverted(self, mock_file, mock_open):
        self.assertMultiLineEqual(
            grep(
                "OF MANS FIRST DISOBEDIENCE, AND THE FRUIT",
                "-i -x -v",
    ...
    # -i + -x + -v: exposes a subtle bug where the -i + -v branch runs
    # first and does a *substring* check, ignoring -x. "FORBIDDEN" is
    # never a full line in paradise-lost.txt, so -x -v -i should return
    # ALL 8 lines. The bug incorrectly excludes line 2 because it
    # contains "Forbidden" as a substring.
    def test_rt_case_insensitive_full_line_inverted_substring(self, mock_file, mock_open):
        self.assertMultiLineEqual(
            grep("FORBIDDEN", "-i -x -v", ["paradise-lost.txt"]),
            "Of Mans First Disobedience, and the Fruit\n"
            "Of that Forbidden Tree, whose mortal tast\n"
            "Brought Death into the World, and all our woe,\n"
            "With loss of Eden, till one greater Man\n"
